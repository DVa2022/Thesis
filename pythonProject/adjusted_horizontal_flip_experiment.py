...
class myOwnDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # the next condition makes sure that the flip happens only if the first and last columns are empty of non-zero
        # pixels. The problem is that because of the lossy compression there are sometimes small values around the
        # corners of the scan cone although the scan cone doesn't reach near the side of the image. That's why I had to
        # make sure that the values are bigger than a threshold of 0.05
        if img[0, :, 0][img[0, :, 0] > 0.05].sum() != 0 and img[0, :, -1][img[0, :, -1] > 0.05].sum() != 0:
            if random.random() < 0.5:
                width = img.size()[2]
                img = torchvision.transforms.functional.hflip(img)
                my_annotation["boxes"] = torch.unsqueeze(torch.tensor([width - xmax - 1, ymin, width - xmin - 1, ymax]), 0)
...
def main():
    ...
    for i in range(0, 10):
        ...
        if experiment_name == 'base':
            experiment_name = experiment_name + '_adjusted_hflip_experiment_' + str(i)
        else:
            experiment_name = 'base' + '_adjusted_hflip_experiment_' + str(i)
