...
def main():
    ...
    gammas = [0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.6, 0.7, 0.8]
    ...
    random_autocontrast_probability = 0.0
    random_equalize_probability = 0.0
    rotation_degrees = 0
    transforms_train = v2.Compose([
        ...
        v2.RandomRotation(degrees=(-1 * rotation_degrees, rotation_degrees),
    ...
    rotation_degrees_options = [0.0, 10, 15, 20, 25]
    best_rotation_degrees = -1
    best_rotation_degrees_performance = -1

    random_autocontrast_probability_options = [0.01, 0.1, 0.15, 0.35, 0.5]
    best_random_autocontrast_probability = -1
    best_random_autocontrast_probability_performance = -1

    random_horizontal_flip_probability_options = [0.1, 0.15, 0.35, 0.5]
    best_random_horizontal_flip_probability = -1
    best_random_horizontal_flip_probability_performance = -1

    random_random_equalize_probability_options = [0.1, 0.15, 0.35, 0.5]
    best_random_equalize_probability = -1
    best_random_equalize_probability_performance = -1

    experiment_name = experiment_name + '_transformations_tuning'

    best_rotation_degrees = 0

    for random_autocontrast_probability in random_autocontrast_probability_options:
        FRCNN_flag = True
        ...
        lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
                                                       step_size=3,
                                                       gamma=0.4)  # step size used to be 3 and gamma 0.1
        ...
        transforms_train = v2.Compose([
            ...
            v2.RandomHorizontalFlip(p=random_horizontal_flip_probability),
            v2.RandomAutocontrast(p=random_autocontrast_probability),
            v2.RandomRotation(degrees=(-1 * best_rotation_degrees, best_rotation_degrees),
                              interpolation=torchvision.transforms.InterpolationMode.NEAREST,
                              expand=False, center=image_center_of_rotation),
            v2.RandomEqualize(p=random_equalize_probability)  # TODO consider applying p=1
            # v2.ToDtype(torch.float32, scale=True),
        ...
        train_loader.dataset.dataset.transforms = transforms_train
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if mAP > best_mAP:
                ...
                if early_stopping_counter == max_epochs_without_improvement:
                    ...
                    if best_mAP > best_random_autocontrast_probability_performance:
                        best_random_autocontrast_probability_performance = best_mAP
                        best_random_autocontrast_probability = random_autocontrast_probability
                    print("best_random_autocontrast_probability_performance is: " + str(best_random_autocontrast_probability_performance))
                    print("best_random_autocontrast_probability is: " + str(best_random_autocontrast_probability))
                    break

    for random_horizontal_flip_probability in random_horizontal_flip_probability_options:
        FRCNN_flag = True
        fine_tuning_flag = True  # if it's true it means that a pretrained model is used rather than training from scratch
        if FRCNN_flag:
            if not fine_tuning_flag:
        ...
        transforms_train = v2.Compose([
            ...
            # v2.RandomResizedCrop(size=(224, 224), antialias=True),
            v2.RandomHorizontalFlip(p=random_horizontal_flip_probability),
            v2.RandomAutocontrast(p=best_random_autocontrast_probability),
            v2.RandomRotation(degrees=(-1 * best_rotation_degrees, best_rotation_degrees),
                              interpolation=torchvision.transforms.InterpolationMode.NEAREST,
                              expand=False, center=image_center_of_rotation),
            v2.RandomEqualize(p=random_equalize_probability)  # TODO consider applying p=1
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if mAP > best_mAP:
                ...
                if early_stopping_counter == max_epochs_without_improvement:
                    ...
                    plt.savefig(experiment_dir + '/performance_per_epoch.png')
                    plt.clf()  # clearing the plot for the next plot
                    if best_mAP > best_random_horizontal_flip_probability_performance:
                        best_random_horizontal_flip_probability_performance = best_mAP
                        best_random_horizontal_flip_probability = random_horizontal_flip_probability
                    break

    for random_equalize_probability in random_random_equalize_probability_options:
        FRCNN_flag = True
        fine_tuning_flag = True  # if it's true it means that a pretrained model is used rather than training from scratch
        if FRCNN_flag:
            if not fine_tuning_flag:
        ...
        transforms_train = v2.Compose([
            ...
            # "Convert a PIL Image or ndarray (H x W x C) to tensor of shape (C x H x W) in the range [0.0, 1.0] if the PIL Image... or if the numpy.ndarray has dtype = np.uint8"
            # v2.RandomResizedCrop(size=(224, 224), antialias=True),
            v2.RandomHorizontalFlip(p=best_random_horizontal_flip_probability),
            v2.RandomAutocontrast(p=best_random_autocontrast_probability),
            v2.RandomRotation(degrees=(-1 * best_rotation_degrees, best_rotation_degrees),
                              interpolation=torchvision.transforms.InterpolationMode.NEAREST,
                              expand=False, center=image_center_of_rotation),
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if mAP > best_mAP:
                ...
                if early_stopping_counter == max_epochs_without_improvement:
                    ...
                    plt.clf()  # clearing the plot for the next plot
                    if best_mAP > best_random_horizontal_flip_probability_performance:
                        best_random_equalize_probability_performance = best_mAP
                        best_random_equalize_probability = random_equalize_probability
                    break
    ...
    for rotation_degrees in rotation_degrees_options:
        FRCNN_flag = True
        fine_tuning_flag = True  # if it's true it means that a pretrained model is used rather than training from scratch
        if FRCNN_flag:
            if not fine_tuning_flag:
        ...
        transforms_train = v2.Compose([
            ...
            v2.RandomRotation(degrees=(-1 * rotation_degrees, rotation_degrees),
                              interpolation=torchvision.transforms.InterpolationMode.NEAREST,
                              expand=False, center=image_center_of_rotation),
            v2.RandomEqualize(p=best_random_equalize_probability)  # TODO consider applying p=1
            # v2.ToDtype(torch.float32, scale=True),
            # v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
        train_loader.dataset.dataset.transforms = transforms_train
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if mAP > best_mAP:
                ...
                if early_stopping_counter == max_epochs_without_improvement:
                    ...
                    plt.savefig(experiment_dir + '/performance_per_epoch.png')
                    plt.clf()  # clearing the plot for the next plot
                    if best_mAP > best_rotation_degrees_performance:
                        best_rotation_degrees_performance = best_mAP
    ...
    better_with_color_jitter = False

    # ****this time with colorjitter****
