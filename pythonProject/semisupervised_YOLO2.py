...
def main():
    ...
    current_ds = os.listdir('/workspace/PycharmProjects/pythonProject/datasets/train2/labels')
    ...
    for file in current_ds:
        if 'hashed' in file and file not in original_ds:
            os.remove('/workspace/PycharmProjects/pythonProject/datasets/train2/labels/' + file)
            os.remove('/workspace/PycharmProjects/pythonProject/datasets/train2/images/' + file.replace('txt', 'jpg'))
    
    max_num_of_frames_from_each_vid_options = [52]
    for max_num_of_frames_from_each_vid in max_num_of_frames_from_each_vid_options:

        images_path = '/workspace/PycharmProjects/pythonProject/thesisData/images/6' if 'workspace' in os.getcwd() else 'thesisData/images/6'
        experiment_name = 'semi-supervised_XL_model_training-series2_' + str(max_num_of_frames_from_each_vid) + '-fold_increase_with_unannotated_data_'
        num_epochs = 55
        ...
        batch_size = 16
        num_workers = 1 if device == torch.device('cpu') else 12
        path2data = "/workspace/PycharmProjects/pythonProject/thesisData"
        path2json = "/workspace/PycharmProjects/pythonProject/thesisData/labels_without_multiple_labels.json"
        ...
        if len(os.listdir('/workspace/PycharmProjects/pythonProject/datasets/train2/labels')) < 10:  # if the folder is empty, it should be filled with data. It happens when max_num_of_frames_from_each_vid==1
            indices, dataset_sizes, _, prev_indices = find_best_unique_split(images_path=images_path, train_share=data_split[0], val_share=data_split[1], test_share=data_split[2], prev_indices=prev_indices)
            train_ds, val_ds, test_ds = random_split(indices, dataset_sizes, full_ds, data_split)
        ...
        semi_supervised_data_leakage_check('2')
        
        num_of_samples = len(os.listdir('/workspace/PycharmProjects/pythonProject/datasets/train2/labels'))
        ...
        for i in range(1):
            ...
            results = model.train(data='/workspace/PycharmProjects/pythonProject/datasets/data2.yaml', epochs=num_epochs, imgsz=640,
                                  patience=max_epochs_without_improvement,
                                  batch=batch_size, cache=True, device=device, workers=num_workers, name=experiment_name,
                                  optimizer='SGD', lr0=0.005, lrf=0.05, degrees=0, translate=0,
                                  scale=0, mosaic=0, mixup=0, fliplr=0, dfl=0, warmup_epochs=0)
